...
namespace parser
{
    ...
    class DllExport ConfigParser
    {
      public:
         ...
         */
        [[nodiscard]] std::shared_ptr<ziapi::config::Node> getValue(std::string const &root, std::string const &module,
         ...
         */
        [[nodiscard]] static const std::vector<LibInfo> getConfigsPaths(std::string const &filename)
        {
            ...
            while (std::getline(file, line)) {
                ...
                if (configPathOffset == std::string::npos)
                    throw std::runtime_error("[ConfigParser] Error while parsing config file");
                libsInfos.emplace_back(LibInfo{line.substr(0, nameEndOffset),
